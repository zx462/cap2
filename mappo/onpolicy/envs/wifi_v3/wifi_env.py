...
class WiFiEnvV2:
    ...
    def __init__(
        ...
        # share_obs: same-link agent obs concat + current avg_SLD +
        #            same-link shared fulfillment vector + link_id
    ...
    def _generate_packets(self):
        ...
        for i in range(self.num_mld):
            self.D[i] = sum(
                np.random.binomial(self.round_length, self.mu[i, l])
    ...
    def _update_sld_after_txop(self, link_id, result, sld_txers):
        ...
        for idx, sld in enumerate(self.sld_state):
            if idx in sld_txers:
                ...
                # Countdown progresses only when a TXOP passes idle.
    ...
    def get_throughput(self):
        ...
        S = self.last_round_link_successes
        ...
        result['throughput/mld_total'] = S.sum() / T
