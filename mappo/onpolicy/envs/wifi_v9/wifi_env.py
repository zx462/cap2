"""WiFi v9 environment with independently scheduled link access opportunities."""
...
class WiFiEnvV9:
    ...
    def __init__(
        ...
        self.max_mld = int(max_mld)
        self.max_sld = int(max_sld)
        if self.max_mld < 1:
            raise ValueError("max_mld must be >= 1")
        if self.max_sld < 0:
            raise ValueError("max_sld must be >= 0")
        if scenario_profile is None:
            scenario_profile = [(self.max_mld, self.max_sld)]
        self.scenario_profile = [
            (int(active_mld), int(active_sld))
            for active_mld, active_sld in scenario_profile
        ]
        if not self.scenario_profile:
            raise ValueError("scenario_profile must contain at least one scenario")
        for active_mld, active_sld in self.scenario_profile:
            if active_mld < 1 or active_mld > self.max_mld:
                raise ValueError(
                    f"active_mld must be in [1, {self.max_mld}], got {active_mld}"
                )
            if active_sld < 0 or active_sld > self.max_sld:
                raise ValueError(
                    f"active_sld must be in [0, {self.max_sld}], got {active_sld}"
                )
        self.scenario_index = 0
        self.active_mld, self.active_sld = self.scenario_profile[self.scenario_index]
        self.num_mld = self.max_mld
        self.num_sld = self.max_sld
        ...
        self.num_agents = self.max_mld * 2

        self.agent_to_mld_link = []
        for mld_id in range(self.max_mld):
        ...
        self.share_obs_dim = self.obs_dim * self.max_mld + 1 + 2 + 2
    ...
    def set_scenario_by_episode(self, episode_idx: int, interval_episodes: int):
        interval = max(int(interval_episodes), 1)
        scenario_index = (int(episode_idx) // interval) % len(self.scenario_profile)
        self.set_scenario_index(scenario_index)

    def set_scenario_index(self, scenario_index: int):
        scenario_index = int(scenario_index) % len(self.scenario_profile)
        if scenario_index == self.scenario_index:
            return
        self.scenario_index = scenario_index
        self.active_mld, self.active_sld = self.scenario_profile[self.scenario_index]
        self.n_sld_per_link = [self.active_sld, 0]

    def _is_active_mld(self, mld_id):
        return mld_id < self.active_mld

    def _is_active_agent(self, aid):
        mld_id, _ = self.agent_to_mld_link[aid]
        return self._is_active_mld(mld_id)

    def _active_link_agents(self, link_id):
        return [aid for aid in self.link_agents[link_id] if self._is_active_agent(aid)]
    ...
    def get_scenario_metrics(self):
        return {
            "scenario/index": float(self.scenario_index),
            "scenario/active_mld": float(self.active_mld),
            "scenario/active_sld": float(self.active_sld),
            "scenario/max_mld": float(self.max_mld),
            "scenario/max_sld": float(self.max_sld),
            "scenario/active_agent_ratio": float(self.active_mld / max(self.max_mld, 1)),
        }
    ...
    def _init_state(self):
        ...
        self.last_ready_links = np.zeros(self.num_links, dtype=bool)
    ...
    def _generate_mu(self):
        ...
        if self.mu_profile is not None:
            if self.mu_profile.shape != (self.max_mld,):
                raise ValueError(
                    f"mu_profile must have length {self.max_mld}, got shape {self.mu_profile.shape}"
                )
            profile = np.clip(self.mu_profile.astype(np.float32), 0.0, 1.0)
            self.mu[: self.active_mld, 0] = profile[: self.active_mld]
            self.mu[: self.active_mld, 1] = profile[: self.active_mld]
            return
        ...
        fixed_mu = np.linspace(mu_min, mu_max, self.active_mld, dtype=np.float32)
        self.mu[: self.active_mld, 0] = fixed_mu
        self.mu[: self.active_mld, 1] = fixed_mu
    ...
    def _get_link_urgencies(self, link_id):
        lacks = {}
        total_lack = 0.0
        for aid in self._active_link_agents(link_id):
            mld_id, _ = self.agent_to_mld_link[aid]
        ...
        if total_lack <= 1e-9:
            return {aid: 0.0 for aid in self._active_link_agents(link_id)}
    ...
    def _decrement_sld_backoff(self):
        for sld in self.sld_state[: self.active_sld]:
            if sld["backoff"] > 0:
                sld["backoff"] -= 1
    ...
    def _get_sld_transmitters(self, link_id):
        ...
        return [
            idx
            for idx, sld in enumerate(self.sld_state[: self.active_sld])
            if sld["backoff"] == 0
        ]
    ...
    def step(self, actions):
        current_ready_links = self.last_ready_links.copy()
        current_step_slots = self.last_step_slots
        ...
        for link_id in range(self.num_links):
            link_aids = self._active_link_agents(link_id)
            if not current_ready_links[link_id]:
                for aid in link_aids:
                    link_result[aid] = "not_ready"
                continue
        ...
        for aid in range(self.num_agents):
            mld_id, link_id = self.agent_to_mld_link[aid]
            is_active = self._is_active_agent(aid)
            infos.append(
                {
                    "active": is_active,
                    "scenario/index": float(self.scenario_index),
                    "scenario/active_mld": float(self.active_mld),
                    "scenario/active_sld": float(self.active_sld),
                    "fulfillment": self._get_fulfillment(mld_id, link_id) if is_active else 0.0,
                    "round_done": done,
                    ...
                    "step_slots": float(current_step_slots),
    ...
    def get_throughput(self):
        ...
        for link_id, link_name in enumerate(["2_4GHz", "5GHz"]):
            mld_success = sum(successes[mld_id, link_id] for mld_id in range(self.active_mld))
        ...
        result["throughput_slot/mld_total"] = active_successes.sum() / total_slots
    ...
    def get_allocation_metrics(self):
        demand = self.last_round_D[: self.active_mld].astype(np.float32)
        success = self.last_round_S[: self.active_mld].astype(np.float32)
        ...
        if total_demand > 0.0:
            ...
            expected_share = np.zeros(self.active_mld, dtype=np.float32)

        if total_success > 0.0:
            actual_share = success / total_success
        else:
            actual_share = np.zeros(self.active_mld, dtype=np.float32)
        ...
        result = {
            ...
            "allocation/abs_gap_mean": float(np.mean(abs_gap)) if len(abs_gap) else 0.0,
            "allocation/match": float(1.0 - np.mean(abs_gap)) if len(abs_gap) else 1.0,
        }
        result.update(self.get_scenario_metrics())
