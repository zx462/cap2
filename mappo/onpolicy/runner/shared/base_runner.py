...
class Runner(object):
    ...
    def __init__(self, config):
        ...
        custom_save_dir = config.get("save_dir")

        if self.use_wandb:
            self.run_dir = str(wandb.run.dir)
            self.save_dir = str(custom_save_dir) if custom_save_dir is not None else str(wandb.run.dir)
            ...
            self.save_dir = str(custom_save_dir) if custom_save_dir is not None else str(self.run_dir / 'models')
            if not os.path.exists(self.save_dir):
                os.makedirs(self.save_dir)
