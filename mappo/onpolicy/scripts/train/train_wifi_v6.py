#!/usr/bin/env python
"""Train MAPPO on the WiFi v6 scenario-rotation environment."""
...
def parse_args(args, parser):
    ...
    parser.add_argument(
        ...
        default="sequential",
        choices=["sequential"],
        help="Scenario order. v6 initially supports sequential order only.",
    )
    parser.add_argument(
...
def main(args):
    ...
    # Compatibility for shared runner metadata that expects num_mld/num_sld.
    ...
    setproctitle.setproctitle(
        f"{all_args.algorithm_name}-wifi-v6-{all_args.experiment_name}"
    )
