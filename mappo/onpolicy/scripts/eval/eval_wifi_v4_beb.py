#!/usr/bin/env python
"""Evaluate a CSMA/CA-style BEB baseline on WiFi v4."""
...
from onpolicy.eval.wifi_v4.mac import MLDBackoffMAC
from onpolicy.eval.wifi_v4.utils import (
    build_eval_run_dir,
...
def parse_args(args, parser):
    ...
    parser.add_argument("--wandb_project", type=str, default="WiFi_v4_eval")
    parser.add_argument("--wandb_group", type=str, default="compare_wifi_v4")
...
def main(args):
    ...
    run_dir = build_eval_run_dir(all_args, "wifi_v4_beb")
    run = init_wandb(all_args, run_dir, "wifi_v4_beb")
    ...
    for episode in range(all_args.eval_episodes):
        ...
        print(f"        action/transmit_ratio={metrics['action/transmit_ratio']:.4f}")
