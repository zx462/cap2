"""Common evaluation utilities for WiFi v4."""
...
def configure_algorithm_flags(args):
    if args.algorithm_name == "rmappo":
        ...
        raise NotImplementedError(
            f"Unsupported algorithm for WiFi v4 evaluation: {args.algorithm_name}"
        )
...
def init_wandb(args, run_dir: Path, eval_name: str):
    ...
    return wandb.init(
        config=vars(args),
        project=getattr(args, "wandb_project", "WiFi_v4_eval"),
...
def compute_episode_metrics(env, infos, episode_reward_total: float):
    ...
    metrics["timing/avg_step_slots"] = float(
        np.mean([info.get("step_slots", 0.0) for info in infos])
    ) if infos else 0.0
...
def print_episode_metrics(episode_idx: int, total_episodes: int, metrics: dict):
    print(
        ...
        f"collision/system_per_slot={metrics['collision_rate/system_per_slot']:.4f} | "
        f"fulfillment={metrics['avg_fulfillment']:.4f} | "
        f"avg_step_slots={metrics['timing/avg_step_slots']:.4f}"
    )
